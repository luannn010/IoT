...
client = None
ser = serial.Serial(
    port='COM6', # Change port to '/dev/ttyACM0' for Linux
    baudrate=9600,
    parity="N",
    stopbits=1,
    bytesize=8
...
def prepareData(msg):
    # Assuming the message format is "flame:<flame_value>,smoke:<smoke_value>"
    data = msg.decode("utf-8").split(',')
    flame_value = data[0].split(':')[1]
    smoke_value = data[1].split(':')[1]
